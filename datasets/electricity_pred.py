...
data = get_dataset("electricity_nips", regenerate=False)
...
# generate samples with window size 48 and stride 8
X_train = sliding_window_generator(X_train, L, L//6)    #(651, 48, 370)

# cut off tails to split into samples of size 48
X_val = X_val[:L*(X_val.shape[0]//L)]                   
X_test = X_test[:L*(X_test.shape[0]//L)]

# split into samples of size 48
X_val = np.array(np.split(X_val, X_val.shape[0]//L))    #(12, 48, 370)
X_test = np.array(np.split(X_test, X_test.shape[0]//L)) #(83, 48, 370)
...
dirname = os.path.join(dirname, 'datasets/electicity_pred/')
