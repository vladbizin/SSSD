...
data = get_dataset("solar_nips", regenerate=False)
...
# generate samples with window size 48 and stride 12
X_train = sliding_window_generator(X_train, L, L//4)    #(522, 48, 137)
...
X_val = np.array(np.split(X_val, X_val.shape[0]//L))    #(14, 48, 137)
X_test = np.array(np.split(X_test, X_test.shape[0]//L)) #(146, 48, 137)
...
dirname = os.path.join(dirname, 'datasets/solar/')
